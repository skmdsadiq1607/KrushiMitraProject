...
export const Dashboard: React.FC = () => {
  ...
  // Chart data: Distribution by problem category
  const categoryData = [
  ...
  // Chart data: Monthly / Activity breakdown
  const monthlyData = [
    { month: 'Jan', scans: 4 },
    { month: 'Feb', scans: 7 },
    { month: 'Mar', scans: 9 },
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      {/* Welcome Banner */}
      <div className="p-6 md:p-8 rounded-2xl bg-gradient-to-r from-emerald-800 to-emerald-950 text-white shadow-lg relative overflow-hidden flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-slate-400 font-semibold uppercase">Category Share</span>
          </div>
        ...
        <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-slate-400 font-semibold uppercase">2025 Trends</span>
          </div>
        ...
        <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between space-y-4">
          <div>
            ...
            {weather ? (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 p-2.5 rounded-xl border border-slate-100 dark:border-slate-800">
                  <div>Humidity: <strong>{weather.current.humidity}%</strong></div>
                  <div>Rainfall: <strong>{weather.current.precipitation} mm</strong></div>
                  <div>Wind: <strong>{weather.current.windSpeed} km/h</strong></div>
                  <div>Risk: <strong className="text-amber-600">{weather.riskAssessment.overallRiskLevel}</strong></div>
                </div>

                {weather.riskAssessment.risks[0] && (
                  <div className="p-2.5 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900 text-xs">
                    <p className="font-bold text-amber-900 dark:text-amber-300">
                      {weather.riskAssessment.risks[0].title}
                    </p>
                    <p className="text-[11px] text-amber-800 dark:text-amber-200 mt-1 line-clamp-2">
                      {weather.riskAssessment.risks[0].description}
                    </p>
                  </div>
                )}
              </div>
            ) : (
              <p className="text-xs text-slate-500">Loading weather telemetry...</p>
            )}
          </div>
          ...
          >
            <span>View Full 5-Day Agro Risk Matrix</span>
      ...
      <div className="p-6 md:p-8 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
        ...
        {history.length > 0 ? (
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 dark:bg-slate-800/60 text-slate-500 font-semibold border-b border-slate-200 dark:border-slate-800">
                <tr>
                  ...
                  <th className="p-3">Problem Identified</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800 font-medium text-slate-700 dark:text-slate-300">
                {history.slice(0, 5).map((item, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="p-3">
                      <SeverityBadge severity={item.severity} isHealthy={item.isHealthy} size="sm" />
                    </td>
          ...
          <div className="text-center py-10 space-y-3">
            ...
            <p className="text-xs text-slate-500">No diagnosis records yet.</p>
            <Link
              to="/diagnose"
