    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="text-center max-w-2xl mx-auto space-y-2">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300 text-xs font-bold">
          ...
          <span>ICAR & Agricultural Extension Standards</span>
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl p-6 md:p-8 space-y-6">
            ...
            {selectedDisease.favorableConditions && (
              <div className="p-3.5 rounded-xl bg-amber-50/60 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 space-y-1.5 text-xs text-amber-900 dark:text-amber-200">
                <p className="font-bold flex items-center gap-1.5">
                  <Thermometer className="w-4 h-4 text-amber-600" />
                  Epidemic Weather Triggers
                </p>
                <p>Temperature: <strong>{selectedDisease.favorableConditions.temperature}</strong></p>
                <p>Humidity: <strong>{selectedDisease.favorableConditions.humidity}</strong></p>
                <p>Environmental Trigger: {selectedDisease.favorableConditions.weatherFactor}</p>
              </div>
            )}
